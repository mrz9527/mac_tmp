...
公式:
    z = np.dot(w.T, x) + b              # z线性回归，  是线性预测结果                   m个样本，m个预测结果z
    a = sigmoid(z)                      # a是激活函数  是激活后的预测结果                m个样本，m个预测结果a
    l = -[ylog(a) + (1-y)log(1-a)]      # l是损失函数，是预测结果与真实结果的偏差          m个样本，m个偏差l
    J = np.sum(l) / m                   # J是成本函数，是偏差的平均值                    m个样本，1个偏差平均值

    dl/dz = a - y                       # l对z偏导，    一个样本一个z                    m个样本，m个偏导
    dz/dw = x                           # z对w偏导，    一个样本，一个W，一个W，n个w       m个样本，m个W，m*n个w
    ...
    dJ/dw1 = (dl1/dw1 + dl2/dw1 + ... + dlm/dw1) /m         dJ/dw1 : m组l对w1的偏导的累加和，没有维度，是一个具体的值
    dJ/dW = [dJ/dw1, dJ/dw2, ..., dJ/dwn]                   dJ/dw:  维度是1*n
"""
...
def forwardTransmit(w, b, X, Y):
    """
    前向传播计算
    约定：多维数组，比如3维数组，维数为3，维度索引从0开始，分别为维度索引分别为0，1，2
        单个数，不是数组，维数为0，没有维度索引
    说明：每一层的每一个神经元（因为这里是单神经元，只有一层神经网络），都对应一套权重和阈值，比如权重(12288, 1)，因为只有一个神经元，所以权重w的索引为1的维度值为1，
        这个值只与该层神经元的个数有关，与样本数无关。同样，阈值b也只与该层神经元个数有关
        所以，训练集样本数m不影响权重的维度，单个训练样本的输入特征数会影响权重w的索引为0的维度的值

        所以，多个训练样本共用一套权重和阈值，不会因为训练样本数量的变化影响权重和阈值维数的变化

    Args:
        w: 单个神经元的输入特征对应的权重维度是(12288, 1)，与单个训练样本的输入特征维度一致
        b:阈值，没有维度，单个值
        X: 单个训练样本的输入特征维度是(12288, 1)，209个训练样本的输入特征维度是(12288, 209)
        Y:单个训练样本的标签没有维度，是单个值，209个训练样本的标签维度是(1, 209)
    ...
    # 前向计算
    m = X.shape[1]                                      # 训练样本数m
    Z = np.dot(w.T, X) + b                              # 线性回归计算：（1，12288）dot (12288, 209) .                                            维度：(1, 209)
    A = sigmoid_function(Z)                             # 激活函数计算：这里a就是预测结果。                                                         维度：(1, 209)，
    L = -[Y * np.log(A) + (1 - Y) * np.log(1 - A) ]     # 单样本损失函数:l = -[ y log(a) + (1-y) log(1-a)]，                                     维度：0
                                                        # m个样本损失函数：L= [l1, l2, ..., lm] = -[ Y log(A) + (1-Y) log(1-A)]                  维度：(1, 209)，
                                                        # Y与对应的A做运算，不是np.dot，是数组对应元素做运算，直接用乘号*

    J = np.sum(L) / m                                   # 成本函数: m个损失函数的均值，[l1 + l2 + l3 + ... + lm] / m，数组L的累加和，然后除以个数m。     维度：0
